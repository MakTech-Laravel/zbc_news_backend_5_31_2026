...
return [
    ...
    /*
     * Retention window for the scheduled `activitylog:purge` command.
     * Each record is removed once it is older than this many months, so the
     * log always holds a rolling window of the most recent months.
     */
    'retention_months' => (int) env('ACTIVITYLOG_RETENTION_MONTHS', 12),

    /*
     * Only used by Spatie's own `activitylog:clean` command, which this app
     * does not schedule. Kept slightly above the retention window so a manual
     * run can never delete more than the policy above allows.
     */
    'clean_after_days' => 366,
